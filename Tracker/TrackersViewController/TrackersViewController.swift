...
extension TrackersViewController {
    private func addTracker(tracker: TrackerModel) {
        self.defaultCategory.trackers.append(tracker)
        self.trackerCreationDates[tracker.id] = datePicker.date
        
        if let index = categories.firstIndex(where: {$0.title == defaultCategory.title}) {
            categories[index] = defaultCategory
        }
        else{
            self.categories.append(defaultCategory)
        }
    ...
    private func addIrregularEvent(tracker: TrackerModel) {
        ...
        self.irregularCategory.trackers.append(tracker)
        self.trackerCreationDates[tracker.id] = datePicker.date
        
        if let index = categories.firstIndex(where: { $0.title == irregularCategory.title }) {
            categories[index] = irregularCategory
        } else {
            self.categories.append(irregularCategory)
        }
